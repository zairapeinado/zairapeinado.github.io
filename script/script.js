...
$(function() {
  ...
  $(window).scroll(function () {
    ...
    var  scrollF = function (){
      ...
      function initSwiper(){
        var initIterator = 0;
        var maxHeight = $('.slider-photo:first').height();
        $('.swiper-container').each(function(){   
          ...
          swipers['swiper-'+index] = new Swiper('.swiper-'+index,{
              ...
              onSlideChangeStart: function(swiper){
                    ...
                    if (typeof onSlideChangeStartCustom === "function") { 
                      onSlideChangeStartCustom();
                    }
              }
          });
          ...
          var adjustSize = function() {
        	  var initialWidth = $t.width();
        	  var initialHeight = maxHeight;
        	  var initialProportion = initialWidth / initialHeight;
        	  var currentMaxHeight = maxHeight;
        	  $t.find('.center-image').each(function(){
            	  var imgWidth = $(this).width();
            	  var imgHeight = $(this).height();
            	  var imgProportion = imgWidth / imgHeight;
            	  var currentHeight = imgHeight;
            	  if ((imgProportion > initialProportion) && (imgWidth > initialWidth)) {
            		currentHeight = imgHeight / (imgWidth/initialWidth);
            	  }
            	  else if ((imgProportion < initialProportion) && (imgHeight > initialHeight)) {
        	  ...
        	  $t.find('.slider-photo').each(function(){
        		  	var img = $(this).find('.center-image:first');
        		  	var imgWidth = img.width();
        		  	var imgHeight = img.height();
            		var resizeHeight = currentMaxHeight;
            		var resizeWidth = (imgWidth * currentMaxHeight) / imgHeight;
        			$(this).width(resizeWidth);
        			$(this).height(resizeHeight);
        			if (initialWidth > resizeWidth){
        				$(this).css('margin-left', ((initialWidth - resizeWidth) / 2) + 'px');
        			}
        	  });
          };
      ...
      $('.arrow-left').on('click', function(){
    	  var slides = $(this).parent().find('.swiper-slide').not('.swiper-slide-duplicate').length;
    	  var swiper = swipers[$(this).parent().attr('data-init')];
          swiper.slideTo(swiper.activeIndex == 1 ? slides : swiper.activeIndex - 1 );
      });

      $('.arrow-right').on('click', function(){
    	  var slides = $(this).parent().find('.swiper-slide').not('.swiper-slide-duplicate').length;
    	  var swiper = swipers[$(this).parent().attr('data-init')];
    	  swiper.slideTo(swiper.activeIndex == slides ? 1 : swiper.activeIndex + 1 );
      });
  ...
  function checkScrollig(){
	...
	$('input, textarea').placeholder(); 
	  ...
	  function resizeCall(){
		pageCalculations();
		$('.swiper-container').each(function(){
		  swipers[$(this).attr('data-init')].update();
		});
	  }
